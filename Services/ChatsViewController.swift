...
class ChatViewController: MessagesViewController {
    ...
    private func insertNewMessage(message: MMessage) {
        ...
        if shouldScrollToBottom {
            DispatchQueue.main.async {
                ...
                print("asdasdasdasdasdASDASFASf")
            }
        }
    }
    ...
    func configureCameraIcon() {
        ...
        cameraItem.tintColor = #colorLiteral(red: 0.5568627451, green: 0.3529411765, blue: 0.9490196078, alpha: 1)
        let cameraImage = UIImage(systemName: "camera", withConfiguration: UIImage.SymbolConfiguration(weight: .regular))
    ...
    private func sendImage(image: UIImage) {
        StorageService.shared.uploadImageMessages(photo: image, to: chat) { (result) in
            switch result {
            ...
            case .success(let url):
                ...
                FirestoreService.shared.sendMessage(chat: self.chat, message: message) { (result) in
                    ...
                    case .failure(_):
                        self.showAlert(title: "Ошибка!", message: "Изображение не было доставлено!")
